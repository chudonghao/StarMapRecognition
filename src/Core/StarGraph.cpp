...
void Convert(double pixel_f,
             ...
             SkySpherePos &to) {
  Vec3d eye = Vec3d(graph_center.pixel_x, graph_center.pixel_y, pixel_f);
  Vec3d from_Q1 = Vec3d(from.pixel_x, special_center.pixel_y, 0.);
  Vec3d from_Q2 = Vec3d(special_center.pixel_x, from.pixel_y, 0.);
  Vec3d special_center_O = Vec3d(special_center.pixel_x, special_center.pixel_y, 0.);
  Vec3d dir_Q1 = from_Q1 - eye;
  Vec3d dir_Q2 = from_Q2 - eye;
  Vec3d dir = special_center_O - eye;

  double sita;
  sita = dir_Q1*dir/dir_Q1.length()/dir.length();
  if (sita > 1.) {
    sita = 1.;
  }
  if (sita < -1.) {
  ...
  double lon = acos(sita);
  sita = dir_Q2*dir/dir_Q2.length()/dir.length();
  if (sita > 1.) {
    sita = 1.;
  }
  if (sita < -1.) {
  ...
  double lat = acos(sita);
  if (from.pixel_x < special_center.pixel_x) {
    lon = -lon;
  }
  if (from.pixel_y < special_center.pixel_y) {
    lat = -lat;
  }
  to.pos.x() = RadiansToDegrees(lon);
  to.pos.y() = RadiansToDegrees(lat);
  to.name = from.name;
}
...
bool StarGraph::InitFrom(const std::string &file_name, double graph_size, double graph_a) {
  ...
  if (!result) {
    LOG_WARNING << "Can NOT read star graph file. file name = " << file_name << " ERROR: " << result.description() << endl;
  }
  vector<StarGraphPos> stars_on_graph;
  for (const auto &row_node :xml_document.select_nodes("//Table/Row")) {
    ...
    if (!name.empty()) {
      StarOnGraph__ star;
      star.SetName(name);
      star.SetStarGraphPos(StarGraphPos(pixel_x, pixel_y));
      stars_[name] = star;
    } else {
      LOG_WARNING << "!name";
    }
  ...
  //for (auto &sog:stars_on_graph) {
  //  SkySpherePos sos_with_special_center;
  //  Convert(pixel_f, StarGraphPos{"", graph_size/2., graph_size/2.}, stars_on_graph[0], sog, sos_with_special_center);
  //LOG_TRACE << sog.name << ":" << sog.pixel_x << ":" << sog.pixel_y << endl;
  //LOG_TRACE << sos_with_special_center.name << ":" << sos_with_special_center.pos.x() << ":" << sos_with_special_center.pos.y() << endl;
  //}
  // test
  //StarGraphPos sog{"", graph_size/2., graph_size/2.};
  //SkySpherePos sos_with_special_center;
  //Convert(pixel_f, sog, StarGraphPos{"", 0., 0.}, StarGraphPos{"", graph_size, graph_size}, sos_with_special_center);
  //LOG_TRACE << sos_with_special_center.name << ":" << sos_with_special_center.pos.x() << ":" << sos_with_special_center.pos.y() << endl;
  //LOG_TRACE << pixel_f << endl;

  // test
  //StarGraphPos sog{"", graph_size/2., graph_size/2.};
  //SkySpherePos sos_with_special_center;
  //Convert(pixel_f, sog, StarGraphPos{"", graph_size-100., 0.}, StarGraphPos{"", graph_size-100., graph_size}, sos_with_special_center);
  ...
  //StarGraphPos sog{"", graph_size/2., graph_size/2.};
  //SkySpherePos sos_with_special_center;
  //Convert(pixel_f, sog, StarGraphPos{"", graph_size/2., 0.}, StarGraphPos{"", graph_size/2., graph_size}, sos_with_special_center);
  //LOG_TRACE << sog.name << ":" << sog.pixel_x << ":" << sog.pixel_y << endl;
  //LOG_TRACE << sos_with_special_center.name << ":" << sos_with_special_center.pos.x() << ":" << sos_with_special_center.pos.y() << endl;
  //LOG_TRACE << pixel_f << endl;

  int max_count = 0;
  for (int i = 0; i < stars_on_graph.size(); ++i) {
    SkySpherePosGroupWithSpecialCenter sos_group;
    CreateStarOnSkySphereGroupWithSpecialCenter(stars_on_graph, pixel_f, StarGraphPos{"", graph_size/2., graph_size/2.}, stars_on_graph[i], sos_group);
    //LOG_TRACE << "=============================" << endl;
    //for (auto &star : sos_group.stars) {
    //  LOG_TRACE << star.name << ":" << star.pos.x() << ":" << star.pos.y() << endl;
    //}
    //LOG_TRACE << "=============================" << endl;
    //for (auto &star : sos_group.stars) {
    //  LOG_TRACE << star.name << ":" << star.pos.x() << ":" << star.pos.y() << endl;
    //}
    //LOG_TRACE << "=============================" << endl;
    //LOG_TRACE << sos_group.max_lon << endl;
    //LOG_TRACE << sos_group.max_lat << endl;
    //LOG_TRACE << sos_group.GetAppropriateRadio() << endl;
    //LOG_TRACE << sos_group.GetAppropriateSize() << endl;
    if (sos_group.size() > max_count) {
      max_count = sos_group.size();
      star_on_sky_sphere_group_[0].swap(sos_group);
    }
  }
  LOG_TRACE << "!!!!!!!!!!!!!!!!!!!!!!!!!!!" << endl;
  LOG_TRACE << "max_count: " << max_count << endl;
...
void CreateStarOnSkySphereGroupWithSpecialCenter(const std::vector<StarTablePos> &stars,
  ...
  for (; pair_iter != lightness_planet_map.end(); --pair_iter) {
    LOG_TRACE << pair_iter->first << endl;
...
Descriptor CreateDescriptor(const SkySpherePosGroupWithSpecialCenter &sos_group) {
  ...
  //LOG_TRACE << tar << endl;
  //LOG_TRACE << pca.project(data) << endl;
  //LOG_TRACE << pca.eigenvectors << endl;
  //LOG_TRACE << pca.mean << endl;
  //LOG_TRACE << pca.eigenvalues << endl;
  ...
  //  LOG_TRACE << endl;
  //}
  //LOG_TRACE << dir.x() << ":" << dir.y() << endl;
  return res;
}

StarGraph::StarGraph() : used_group_count(1) {}

Descriptor StarGraph::GetDescriptor() {
  return CreateDescriptor(star_on_sky_sphere_group_[0]);
}
